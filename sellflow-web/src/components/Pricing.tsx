    ...
    <>
          ...
          <div className="relative mt-14 grid gap-5 md:grid-cols-2 lg:mt-16 lg:grid-cols-3 lg:gap-6">
            {plans.map((plan) => (
              <article
                key={plan.id}
                className={[
                  "relative flex h-full flex-col rounded-xl border bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-7",
                  plan.highlighted
              ...
              >
                {plan.highlighted && (
                  <span className="absolute right-6 top-0 -translate-y-1/2 rounded-xl bg-violet-600 px-3 py-1 text-xs font-semibold text-white shadow-lg shadow-violet-200">
                    Most popular
                  </span>
                )}
                ...
                <button
                  ...
                  className={[
                    "mt-7 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-semibold transition",
                    plan.highlighted
                ...
                <ul className="mt-7 flex-1 space-y-3">
                  {plan.features.map((feature) => (
                    ...
                    >
                      <div className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-xl bg-violet-50 text-violet-600">
      ...
      {selectedPlan && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/65 p-3 backdrop-blur-sm sm:p-4">
          <div className="relative w-full max-w-sm max-h-[calc(100dvh-1.5rem)] overflow-y-auto rounded-xl bg-white shadow-2xl [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:max-h-[calc(100dvh-2rem)]">
            <button
              type="button"
              onClick={closePayment}
              className="absolute right-3 top-3 z-10 rounded-xl bg-white/90 p-2 text-slate-500 shadow-sm transition hover:bg-slate-100 hover:text-slate-900"
            ...
            {paymentStatus === "idle" && (
              <div className="px-4 pb-4 sm:px-5">
                ...
                <button
                  ...
                  className="mt-3 w-full rounded-xl bg-purple-500 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-purple-600"
                >
            ...
            {paymentStatus === "success" && (
              <div className="px-6 py-10 text-center">
                <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-xl bg-emerald-50 text-emerald-600">
