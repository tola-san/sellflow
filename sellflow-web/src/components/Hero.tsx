      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-4 px-4 sm:px-6 lg:grid-cols-[.95fr_1.05fr] lg:gap-8 lg:px-8">
        <div className="mx-auto max-w-2xl text-center lg:mx-0 lg:text-left">
          ...
          >
            ...
            Social selling, now on autopilot
          </motion.div>

          <motion.h1
            initial={{ opacity: 0, y: 22 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.65, delay: 0.05 }}
            className="mt-6 text-4xl font-semibold leading-[1.04] tracking-[-0.045em] text-slate-950 sm:text-5xl lg:text-[4rem]"
          >
            Sell everywhere.
            <span className="block bg-gradient-to-r from-violet-700 via-fuchsia-500 to-blue-500 bg-clip-text text-transparent">Manage it in one flow.</span>
          </motion.h1>
          ...
          >
            Connect your social channels, capture every order, and automate customer conversations from one beautifully simple workspace.
          </motion.p>
          ...
          >
            <Link to="/register" className="group inline-flex items-center justify-center gap-2 rounded-lg bg-slate-950 px-6 py-3.5 text-sm font-semibold text-white shadow-xl shadow-slate-300 transition hover:-translate-y-0.5 hover:bg-violet-700">
              Start selling free <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" />
            </Link>
            <a href="#features" className="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white/80 px-6 py-3.5 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur transition hover:-translate-y-0.5 hover:border-violet-200 hover:text-violet-700">
          ...
          >
            {trustPoints.map((point) => (
              <span key={point} className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-500 sm:text-sm">
                <Check className="h-3.5 w-3.5 text-emerald-500" /> {point}
              </span>
            ))}
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="mt-8 hidden items-center gap-3 text-xs font-medium text-slate-400 lg:flex">
            <Bot className="h-4 w-4 text-violet-500" /> Built to automate the busywork, not your relationships.
          </motion.div>
        </div>
